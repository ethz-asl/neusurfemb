...
class Scene:
    ...
    def __init__(self, scene_path):
        ...
        # NOTE: It is assumed that undistortion has already been performed!
        self.rgb_path = os.path.join(scene_path, 'rgb')
        self.depth_path = os.path.join(scene_path, 'depth')

        # Read intrinsics.
        self.K = np.loadtxt(os.path.join(scene_path,
                                         "intrinsics.txt")).reshape(3, 3)
        try:
            D = np.loadtxt(os.path.join(
                scene_path, "distortion_parameters.txt")).reshape(-1)
...
class HLoc:
    ...
    def _run_sfm(self):
        image_dir = Path(self.scene.rgb_path)
        image_list = []
        ...
        image_options = {
            'camera_model':
                "PINHOLE",
            'camera_params': (f"{self.scene.K[0, 0]}, {self.scene.K[1, 1]}, "
                              f"{self.scene.K[0, 2]}, {self.scene.K[1, 2]}"),
        }
        mask_dir = image_dir.parent / 'mask'

        if (mask_dir.exists()):
            image_options['mask_path'] = str(mask_dir)
            print(f"\033[93mUsing mask images in '{str(mask_dir)}' to mask out "
                  "features within the given per-image masks.\033[0m")
        ...
        (focal_length_x, focal_length_y, c_x,
         c_y) = self.model.cameras[1].params
        ...
        assert (np.all(self.colmap_K == self.scene.K))
    ...
    def save_matches(self):
        ...
        for image in self.hloc.model.images.values():
            ...
            image_to_points_3D[image.name] = copy.deepcopy({
                'pixel_to_point3D_id': pixel_to_point3D_id,
                'point3D_id_to_pixel': point3D_id_to_pixel
            })
        # Save to disk.
